      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className={`grid gap-2 mb-4 ${cards.length === 1 ? 'grid-cols-1 max-w-[140px] mx-auto' : 'grid-cols-5 max-w-3xl mx-auto'}`}>
          {cards.map((card, index) => (
            ...
            >
              <div className={`relative rounded-lg border-2 ${getRarityBorder(card.rarity)} shadow-xl aspect-[2/3] overflow-hidden bg-gray-900`}>
                {card.image ? (
                ...
                <div className="absolute bottom-0 left-0 right-0 z-10 p-1.5">
                  <div className="bg-black/60 backdrop-blur-sm rounded px-1.5 py-0.5 border border-white/20">
                    <p className="text-white text-center font-black text-[10px] leading-tight drop-shadow-2xl tracking-wide truncate">{card.name}</p>
                  </div>
                </div>
