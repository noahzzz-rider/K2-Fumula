...
function CardFlip({ isFlipped, size, card, delay = 0 }: CardFlipProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`relative w-full h-full bg-gradient-to-br ${getRarityColor(card.rarity)} rounded-2xl border-4 ${getRarityBorder(card.rarity)} shadow-2xl overflow-hidden flex flex-col items-center justify-between ${size === 'large' ? 'p-6' : 'p-3'}`}>
          ...
          <div className="absolute top-0 left-0 w-full h-full opacity-40">
            <div className={`absolute ${size === 'large' ? 'top-3 left-3 w-10 h-10' : 'top-2 left-2 w-7 h-7'} border-t-3 border-l-3 border-white/60 rounded-tl-xl`}></div>
            <div className={`absolute ${size === 'large' ? 'top-3 right-3 w-10 h-10' : 'top-2 right-2 w-7 h-7'} border-t-3 border-r-3 border-white/60 rounded-tr-xl`}></div>
            <div className={`absolute ${size === 'large' ? 'bottom-3 left-3 w-10 h-10' : 'bottom-2 left-2 w-7 h-7'} border-b-3 border-l-3 border-white/60 rounded-bl-xl`}></div>
            <div className={`absolute ${size === 'large' ? 'bottom-3 right-3 w-10 h-10' : 'bottom-2 right-2 w-7 h-7'} border-b-3 border-r-3 border-white/60 rounded-br-xl`}></div>
          </div>

          {/* 内边框 */}
          <div className={`absolute ${size === 'large' ? 'inset-5' : 'inset-3'} border-2 border-white/20 rounded-xl pointer-events-none`}></div>
          ...
          <div className="flex-1 flex items-center justify-center z-10">
            <div className="relative">
              <div className="absolute inset-0 bg-white/30 rounded-full blur-3xl scale-[2]"></div>
              ...
              <motion.div
                ...
                className={`relative ${size === 'large' ? 'text-8xl' : 'text-5xl'} drop-shadow-2xl`}
              >
                {card.emoji}
              </motion.div>
            </div>
          ...
          <div className="w-full z-10">
            ...
            <div className="bg-black/20 backdrop-blur-sm rounded-lg px-2 py-1.5 border border-white/20">
              <p className={`text-white text-center font-black ${size === 'large' ? 'text-xl' : 'text-xs'} leading-tight drop-shadow-2xl tracking-wide`}>
                {card.name}
              </p>
            </div>
          </div>
